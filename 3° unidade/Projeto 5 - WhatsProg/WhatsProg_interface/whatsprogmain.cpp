...
void WhatsProgMain::on_actionApagar_mensagens_triggered()
{
    ...
    QLabel *provMsg;
    QString nMensagens = QString::number(it->getNumMsgsEntregues()) + "/" + QString::number(it->size());
    provMsg = new QLabel(nMensagens);
    provMsg->setAlignment(Qt::AlignCenter);
    ui->tableConversas->setCellWidget(it, 0, provMsg);

    QLabel *provDest;
    QString destinatario = it->getCorrespondente().c_str();
    provDest = new QLabel(destinatario);
    provDest->setAlignment(Qt::AlignCenter);
    ui->tableConversas->setCellWidget(it, 1, provDest);
}
*/
...
void WhatsProgMain::on_tableConversas_cellClicked(int row, int)
{
    ...
    string destinatario = ui->tableConversas->item(row, 1)->text().toStdString();
...
void WhatsProgMain::listarMensagens(IterConversa it)
{
    ...
    for(unsigned i=0; i<it->size(); i++){
        ...
        if(it->getMensagem(i).getRemetente() == DC.getMeuUsuario()){
            ...
            prov->setAlignment(Qt::AlignRight);
