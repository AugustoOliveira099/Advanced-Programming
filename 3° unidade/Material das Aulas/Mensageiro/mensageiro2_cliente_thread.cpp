...
void Mensageiro2_Cliente_Thread::main_thread()
{
  ...
  if (em_execucao && erro)
  {
    ...
    emit signErro("Erro na recepcao", "Conexao interrompida abruptamente");
  }
  ...
  emit signThreadEncerrada();
}

/// A funcao main_thread() executa o metodo main_thread()
void main_thread(Mensageiro2_Cliente_Thread *mct) {mct->main_thread();}
