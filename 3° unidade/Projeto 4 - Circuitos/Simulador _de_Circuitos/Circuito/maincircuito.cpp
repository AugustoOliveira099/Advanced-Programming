...
void maincircuito::ExibirTabelasIniciais()
{
    // Dados para o tamanho das tabelas
    unsigned nEntradas, nSaidas, nPortas;
    QLabel *prov;
    ...
    for(unsigned i=0; i<nPortas; i++){
        ...
        prov = new QLabel("??");
        prov->setAlignment(Qt::AlignCenter);
        ui->portasConexoesWidget->setCellWidget(i, 0, prov);

        // SEGUNDA COLUNA (N° ENTRADAS)
        prov = new QLabel("0");
        prov->setAlignment(Qt::AlignCenter);
        ui->portasConexoesWidget->setCellWidget(i, 1, prov);

        // COLUNAS ADIANTES (ENTRADAS)
        for(unsigned j=0; j<nEntradas; j++){
            prov = new QLabel("");
            prov->setAlignment(Qt::AlignCenter);
            ui->portasConexoesWidget->setCellWidget(i, 2+j, prov);
        }
    }
    ...
    for(unsigned i=0; i<nSaidas; i++){
        ...
        prov = new QLabel("0");
        prov->setAlignment(Qt::AlignCenter);
        ui->saidasWidget->setCellWidget(i, 0, prov);
    }
...
void maincircuito::ExibirPorta(unsigned idPorta)
{
    ...
    for(unsigned i=1; i<=nEntradas; i++){
        id[i-1] = C.getId_inPort(idPorta, i);
    }

    // PRIMEIRA COLUNA (TIPO DE PORTA)
    prov = new QLabel(tipoPorta.c_str());
    prov->setAlignment(Qt::AlignCenter);
    ui->portasConexoesWidget->setCellWidget(idPorta-1, 0, prov);

    // SEGUNDA COLUNA (N° ENTRADAS)
    prov = new QLabel(QString::number(nEntradas));
    prov->setAlignment(Qt::AlignCenter);
    ui->portasConexoesWidget->setCellWidget(idPorta-1, 1, prov);

    // COLUNAS ADIANTES (ENTRADAS)
    for(unsigned i=0; i<nEntradas; i++){
        prov = new QLabel(QString::number(id[i]));
        prov->setAlignment(Qt::AlignCenter);
        ui->portasConexoesWidget->setCellWidget(idPorta-1, 2+i, prov);
    }
}
...
void maincircuito::slotModifyPorta(QString idPorta, QString tipoPorta, QString nEntradas, QString id[4])
{
    ...
    unsigned newNEntradas = nEntradas.toUInt();
    int newId[newNEntradas];

    for(int i=0; i<4; i++) qDebug() << "Teste SLOT: " << id[i].toInt();

        for(int i=0; i<int(newNEntradas); i++){
            newId[i] = id[i].toInt();
        }
    // C.setPort(idPorta, nomePorta, nEntradas)
    C.setPort(newIdPorta, newTipoPorta, newNEntradas);

    for(unsigned i=1; i<=newNEntradas; i++){
        C.setId_inPort(newIdPorta, i-1, newId[i-1]);
    }

    ExibirPorta(newIdPorta);
}

void maincircuito::slotModifySaida(QString idSaida, QString idOrigem)
{
    unsigned newIdSaida = idSaida.toUInt()-1;
    int newIdOrigem = idOrigem.toInt();

    C.setIdOutput(newIdSaida, newIdOrigem);

    ExibirSaida(newIdSaida);
}
