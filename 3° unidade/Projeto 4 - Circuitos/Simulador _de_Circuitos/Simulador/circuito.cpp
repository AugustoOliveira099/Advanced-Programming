...
bool Circuit::valid() const
{
    ...
    if((getNumPorts()==0) && (getNumInputs()>getNumOutputs())) return false; // Haverah inputs inutilizadas

    for(unsigned i=0; i<getNumPorts(); i++)
    {
        tipo = getNamePort(i+1);

        if( (tipo!="NT") && (tipo!="AN") && (tipo!="NA") && (tipo!="OR") && (tipo!="NO") && (tipo!="XO") && (tipo!="NX") )
            return false;

        if(getNamePort(i+1)=="NT")
        {
            if((*ports[i]).getNumInputs()!=1) return false;

        }
        else if( ((*ports[i]).getNumInputs()<2) || ((*ports[i]).getNumInputs()>4) ) return false;
    ...
    {
        if( (getIdOutput(i)==0) || (getIdOutput(i)>int(getNumPorts())) || (getIdOutput(i)<(-int(getNumInputs()))) ) return false;
    }
...
{
    if((T!=getNamePort(IdPort)) && (IdPort<=getNumPorts()) && (NIn>0) && (NIn<5))
    {
        if(T=="NT")
        {
            Port_NOT arr;
            ports[IdPort-1] = (&arr)->clone();
            if(NIn==1) (*ports[IdPort-1]).setNumInputs(NIn);
        }
        else if(T=="AN")
        {
            Port_AND arr;
...
void Circuit::digitar()
{
    ...
    for(unsigned i=1; i<=portas; i++)
    {
        ...
        else if(tipo=="AN")
        {
            ...
            (*ports[i-1]).setNumInputs(getNumInputsPort(i));
        }
        else if(tipo=="NA")
        {
            Port_NAND arr;
