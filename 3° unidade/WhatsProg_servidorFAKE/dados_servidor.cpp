  ...
  if (iDest->connected()) while(M!=LM.end())
  {
    ...
    if(M!=LM.end())
    {
      ...
      if (envioOK)
      {
        ...
        iterUsuario iRemet = find(LU.begin(), LU.end(), M->getRemetente());
        if(iRemet!=LU.end())
        {
          // Remetente existe
...
void DadosServidor::enviarConfirmacoesParaUsuario(iterUsuario iRemet)
{
  list<Mensagem>::iterator *excluir = nullptr;
  list<Mensagem>::iterator M = LM.begin();
  ...
  if(iRemet->connected()) while(M!=LM.end())
  {
  ...
  if (iRemet->connected()) while(M!=LM.end())
  {
    ...
    if(M!=LM.end())
    {
      ...
      else
      {
        ...
        // Cria array de iteradores apontando para as mensagens a serem excluidas
        list<Mensagem>::iterator *aux = new list<Mensagem>::iterator [N+1];
        for(unsigned i=0; i<N; i++) aux[i] = excluir[i];
        aux[N] = M;
        if(aux!=nullptr) delete[] excluir;
        excluir = aux;
        N++;
      }
  ...
  for(unsigned i=0; i<N; i++) LM.erase(excluir[i]);
}
          ...
          if (!fim && iUsuario->connected() && f.had_activity(iUsuario->getSocket()))
          {
            ...
            if (iUsuario->connected()) switch(cmd)
              ...
              case CMD_NOVA_MSG:
              {
                ...
                string destinatario;
                string texto;
                iterUsuario iUsuario_prov;

                // Receber a msg
                iResult = iUsuario->read_int(id, TIMEOUT_WHATSPROG*1000);
                if (iResult == mysocket_status::SOCK_OK)
                {
                  iResult = iUsuario->read_string(destinatario, TIMEOUT_WHATSPROG*1000);
                }
                if (iResult == mysocket_status::SOCK_OK)
                {
                  iResult = iUsuario->read_string(texto, TIMEOUT_WHATSPROG*1000);
                }
                if (iResult != mysocket_status::SOCK_OK)
                {
                  // Pode ser mysocket_status::SOCK_TIMEOUT, mysocket_status::SOCK_DISCONNECTED ou mysocket_status::SOCK_ERRO
              ...
              case CMD_MSG_LIDA1:
              {
                ...
                iResult = iUsuario->read_int(id, TIMEOUT_WHATSPROG*1000);
                if (iResult  == mysocket_status::SOCK_OK) iResult = iUsuario->read_string(remetente, TIMEOUT_WHATSPROG*1000);
                if (iResult  != mysocket_status::SOCK_OK)
                {
                  // Pode ser mysocket_status::SOCK_TIMEOUT, mysocket_status::SOCK_DISCONNECTED ou mysocket_status::SOCK_ERRO
                  if (iResult == mysocket_status::SOCK_ERROR)
                ...
                if (iUsuario->connected())
                {
                  ...
                  iterUsuario remetente_prov = find(LU.begin(), LU.end(), M->getRemetente());
...
{
  c.close();
  for(iterUsuario user=LU.begin(); user!=LU.end(); user++)
    user->close();
}
